    ...
    <figure className='property'>
        ...
        <figcaption>
            <main className='propertydetails'>
                <h5>{name}</h5>
                <h6><span className='metarial-symbols-outlined houseicon'>home_pin</span><address></address></h6>
                <p>
                    <span className='price'>Rs{price}</span> One day
                </p>
            </main>
        </figcaption>
    </figure>
...
useEffect(()=>{
    if(propertyListRef.current){
        gsap.fromTo(propertyListRef.current.children,{y:50,opacity:0},{y:0,opacity:1,duration:0.6,ease:"power2.out",stagger:0.1});
    }
},[properties]);

return(
    <>
    {properties.length===0?(<p className='not_found'>Not found</p>):(
        <div className='propertylist' ref={propertyListRef}>
            {properties.map((property)=>(<Card key={property._id} id={property._id}
            image={property.images[0].url} name={property.propertyName} 
