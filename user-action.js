...
export const getSignup = (user) => async (dispatch) => {
    try {
        ...
        const response = await axiosInstance.post("/v1/rent/user/signup", user);
        dispatch(userActions.getSignupDetails(data.user));
    } catch (error) {
        dispatch(userActions.getError(error.response.data.error));
    }
}
...
export const updateUser = (updateUser) => async (dispatch) => {
    try{
        ...
        const response = await axiosInstance.put("/v1/rent/user/updateMe", updateUser);
        console.log("Update successfull", response.data);
        const {data} = await axiosInstance.put("/v1/rent/user/me");
        dispatch(userActions.getCurrentUser(data.user));
    }
    catch(error){
        dispatch(userActions.getError(error.response.data.message));
